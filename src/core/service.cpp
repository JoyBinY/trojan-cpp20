...
Service::Service(Config &config, bool test)
    : config(config), socket_acceptor(io_context), ssl_context(context::sslv23),
      auth(nullptr), udp_socket(io_context) {
#ifndef ENABLE_NAT
  ...
  if (!test) {
    ...
    tcp::endpoint listen_endpoint =
        *resolver.resolve(config.local_addr, to_string(config.local_port))
             .begin();
    ...
    if (config.run_type == Config::FORWARD) {
      auto udp_bind_endpoint =
          udp::endpoint(listen_endpoint.address(), listen_endpoint.port());
  ...
  if (config.run_type == Config::SERVER) {
    ...
    ssl_context.set_password_callback(
        [this](size_t, context_base::password_purpose) {
    ...
    if (config.ssl.dhparam.empty()) {
      ssl_context.use_tmp_dh(asio::const_buffer(SSLDefaults::g_dh2048_sz,
...
void Service::run() {
  ...
  Log::log_with_date_time(format("trojan service ({}) started at {}:{}", rt, local_endpoint.address().to_string(), local_endpoint.port()), Log::WARN);
  io_context.run();
...
void Service::async_accept() {
  ...
  socket_acceptor.async_accept(
      session->accept_socket(), [this, session](const asio::error_code error) {
...
void Service::udp_async_read() {
  udp_socket.async_receive_from(
      asio::buffer(udp_read_buf, MAX_LENGTH), udp_recv_endpoint,
      [this](const asio::error_code error, size_t length) {
        if (error == asio::error::operation_aborted) {
          // got cancel signal, stop calling myself
          return;
        }
        ...
        Log::log_with_endpoint(tcp::endpoint(udp_recv_endpoint.address(),
                                             udp_recv_endpoint.port()),
                               "new UDP session");
        auto session = make_shared<UDPForwardSession>(
            config, io_context, ssl_context, udp_recv_endpoint,
            [this](const udp::endpoint &endpoint, const string &data) {
              ...
              if (ec == asio::error::no_permission) {
                Log::log_with_endpoint(
                    ...
                    "dropped a UDP packet due to firewall policy or rate "
                    "limit");
              } else if (ec) {
...
asio::io_context &Service::service() { return io_context; }
