...
NATSession::NATSession(const Config &config, asio::io_context &io_context, context &ssl_context) :
...
pair<string, uint16_t> NATSession::get_target_endpoint() {
    ...
    if (destaddr.ss_family == AF_INET) {
        auto *sa = reinterpret_cast<sockaddr_in*>(&destaddr);
        inet_ntop(AF_INET, &(sa->sin_addr), ipstr, INET_ADDRSTRLEN);
        port = ntohs(sa->sin_port);
    } else {
        auto *sa = reinterpret_cast<sockaddr_in6*>(&destaddr);
...
void NATSession::start() {
    ...
    auto [target_addr, target_port] = get_target_endpoint();
    if (target_port == 0) {
        destroy();
        return;
    }
    ...
    in_async_read();
    Log::log_with_endpoint(in_endpoint, "forwarding to " + target_addr + ':' + to_string(target_port) + " via " + config.remote_addr + ':' + to_string(config.remote_port), Log::Level::INFO);
    auto self = shared_from_this();
    resolver.async_resolve(config.remote_addr, to_string(config.remote_port), [this, self](const std::error_code error, const tcp::resolver::results_type& results) {
        ...
        out_socket.next_layer().async_connect(*iterator, [this, self](const std::error_code error) {
            if (error) {
            ...
            out_socket.async_handshake(stream_base::client, [this, self](const std::error_code error) {
                if (error) {
...
void NATSession::in_async_write(string_view data) {
    auto self = shared_from_this();
...
void NATSession::out_async_write(string_view data) {
    auto self = shared_from_this();
    auto data_copy = make_shared<string>(data);
...
void NATSession::in_recv(string_view data) {
...
void NATSession::out_recv(string_view data) {
    if (status == Status::FORWARD) {
        recv_len += data.size();
        in_async_write(data);
    }
