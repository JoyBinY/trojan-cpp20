...
ServerSession::ServerSession(const Config &config, asio::io_context &io_context, context &ssl_context, Authenticator *auth, const string &plain_http_response) :
...
tcp::socket& ServerSession::accept_socket() {
    return static_cast<tcp::socket&>(in_socket.next_layer());
}
...
void ServerSession::start() {
    ...
    in_socket.async_handshake(stream_base::server, [this, self](const std::error_code error) {
        if (error) {
            Log::log_with_endpoint(in_endpoint, "SSL handshake failed: " + error.message(), Log::Level::ERROR);
            if (error.message() == "http request" && !plain_http_response.empty()) {
                recv_len += plain_http_response.length();
                asio::async_write(accept_socket(), asio::buffer(plain_http_response), [this, self](const std::error_code, size_t) {
                    destroy();
                });
                return;
            }
...
void ServerSession::in_async_read() {
    auto self = shared_from_this();
    in_socket.async_read_some(asio::buffer(in_read_buf.data(), MAX_LENGTH), [this, self](const std::error_code error, size_t length) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::in_async_write(string_view data) {
    auto self = shared_from_this();
    auto data_copy = make_shared<string>(data);
    asio::async_write(in_socket, asio::buffer(*data_copy), [this, self, data_copy](const std::error_code error, size_t) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::out_async_read() {
    auto self = shared_from_this();
    out_socket.async_read_some(asio::buffer(out_read_buf.data(), MAX_LENGTH), [this, self](const std::error_code error, size_t length) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::out_async_write(string_view data) {
    auto self = shared_from_this();
    auto data_copy = make_shared<string>(data);
    asio::async_write(out_socket, asio::buffer(*data_copy), [this, self, data_copy](const std::error_code error, size_t) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::udp_async_read() {
    auto self = shared_from_this();
    udp_socket.async_receive_from(asio::buffer(udp_read_buf.data(), MAX_LENGTH), udp_recv_endpoint, [this, self](const std::error_code error, size_t length) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::udp_async_write(string_view data, const udp::endpoint &endpoint) {
    auto self = shared_from_this();
    auto data_copy = make_shared<string>(data);
    udp_socket.async_send_to(asio::buffer(*data_copy), endpoint, [this, self, data_copy](const std::error_code error, size_t) {
        if (error) {
            destroy();
            return;
        }
...
void ServerSession::in_recv(string_view data) {
    if (status == Status::HANDSHAKE) {
        TrojanRequest req;
        bool valid = req.parse(string(data)) != -1;
        if (valid) {
            ...
            if (password_iterator == config.password.end()) {
                ...
                if (auth && auth->auth(req.password)) {
                    ...
                    Log::log_with_endpoint(in_endpoint, "authenticated by authenticator (" + req.password.substr(0, 7) + ')', Log::Level::INFO);
                }
            } else {
                Log::log_with_endpoint(in_endpoint, "authenticated as " + password_iterator->second, Log::Level::INFO);
            }
            if (!valid) {
                Log::log_with_endpoint(in_endpoint, "valid trojan request structure but possibly incorrect password (" + req.password + ')', Log::Level::WARN);
            }
        }
        ...
        if (valid) {
            ...
            if (req.command == TrojanRequest::Command::UDP_ASSOCIATE) {
                Log::log_with_endpoint(in_endpoint, "requested UDP associate to " + req.address.address + ':' + to_string(req.address.port), Log::Level::INFO);
            ...
            Log::log_with_endpoint(in_endpoint, "not trojan request, connecting to " + query_addr + ':' + query_port, Log::Level::WARN);
            out_write_buf = string(data);
        }
        sent_len += out_write_buf.length();
        auto self = shared_from_this();
        resolver.async_resolve(query_addr, query_port, [this, self, query_addr, query_port](const std::error_code error, const tcp::resolver::results_type& results) {
            if (error || results.empty()) {
                Log::log_with_endpoint(in_endpoint, "cannot resolve remote server hostname " + query_addr + ": " + error.message(), Log::Level::ERROR);
                destroy();
                return;
            }
            ...
            Log::log_with_endpoint(in_endpoint, query_addr + " is resolved to " + iterator->endpoint().address().to_string(), Log::Level::ALL);
            std::error_code ec;
            ...
            if (config.tcp.keep_alive) {
                out_socket.set_option(asio::socket_base::keep_alive(true));
            }
            ...
            out_socket.async_connect(*iterator, [this, self, query_addr, query_port](const std::error_code error) {
                if (error) {
                    Log::log_with_endpoint(in_endpoint, "cannot establish connection to remote server " + query_addr + ':' + query_port + ": " + error.message(), Log::Level::ERROR);
                    destroy();
                    return;
                }
...
void ServerSession::out_recv(string_view data) {
    if (status == Status::FORWARD) {
        recv_len += data.size();
        in_async_write(data);
    }
...
void ServerSession::udp_recv(string_view data, const udp::endpoint &endpoint) {
...
void ServerSession::udp_sent() {
    if (status == Status::UDP_FORWARD) {
        ...
        auto self = shared_from_this();
        udp_resolver.async_resolve(query_addr, to_string(packet.address.port), [this, self, packet, query_addr](const std::error_code error, const udp::resolver::results_type& results) {
            if (error || results.empty()) {
                Log::log_with_endpoint(in_endpoint, "cannot resolve remote server hostname " + query_addr + ": " + error.message(), Log::Level::ERROR);
                destroy();
                return;
...
void ServerSession::destroy() {
    ...
    if (in_socket.next_layer().is_open()) {
        auto self = shared_from_this();
        auto ssl_shutdown_cb = [this, self](const std::error_code error) {
